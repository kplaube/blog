...
def content_object_init(instance):
    ...
    def _get_content(self):
        ...
        if self.settings['SUMMARY_BEGIN_MARKER']:
            content = content.replace(self.settings['SUMMARY_BEGIN_MARKER'],
                                      '', 1)
        if self.settings['SUMMARY_END_MARKER']:
            content = content.replace(self.settings['SUMMARY_END_MARKER'], '',
                                      1)
        return content
    ...
    if not hasattr(instance, '_summary') and instance._content is not None:
        ...
        if instance.settings['SUMMARY_BEGIN_MARKER']:
            begin_summary = content.find(
                instance.settings['SUMMARY_BEGIN_MARKER'])
        ...
        if begin_summary != -1 or end_summary != -1:
            ...
            # of the marker
            begin_summary = (
                begin_summary + len(instance.settings['SUMMARY_BEGIN_MARKER'])
                if begin_summary != -1 else 0)
            end_summary = end_summary if end_summary != -1 else None
            instance._summary = instance._update_content(
                content[begin_summary:end_summary],
